...
public class SistemaFilaBanco {
    ...
    public void chamarCliente() {
        if (prioridadeCounter < 2 && !filaPrioritaria.isEmpty()) {
            ...
            prioridadeCounter++;
            JOptionPane.showMessageDialog(null, "Chamando cliente prioritário: " + cliente.getNome() + " (Senha: " + cliente.getSenha() + ")");
        } else if (!filaNormal.isEmpty()) {
            Cliente cliente = filaNormal.dequeue();
            prioridadeCounter = 0;
            JOptionPane.showMessageDialog(null, "Chamando cliente normal: " + cliente.getNome() + " (Senha: " + cliente.getSenha() + ")");
